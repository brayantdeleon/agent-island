...
public struct KimiHookInstallerManifest: Equatable, Codable, Sendable {
    public static let fileName = "agent-island-kimi-hooks-install.json"
...
/// Installs/uninstalls Agent-Island's managed `[[hooks]]` entries in
/// `~/.kimi/config.toml`.
///
...
public enum KimiHookInstaller {
    public static let markerComment = "# agent-island: managed hook — do not edit"
    ...
    /// Agent-Island / Vibe Island build.
    ...
    private static func blockMatchesManagedCommand(_ block: [String], managedCommand: String?) -> Bool {
        for line in block {
            ...
            if isLegacyAgentIslandHookCommand(value) {
                return true
            }
        }
        return false
    ...
    /// escape handling is unnecessary — Agent-Island only ever writes paths,
