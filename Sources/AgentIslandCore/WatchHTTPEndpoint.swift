...
///
/// Uses `NWListener` for TCP + Bonjour advertising of `_agentisland._tcp`.
...
public final class WatchHTTPEndpoint: @unchecked Sendable {
    private static let logger = Logger(subsystem: "app.agentisland", category: "WatchHTTPEndpoint")
    private static let serviceType = "_agentisland._tcp"
    private static let pairingCodeLength = 4
    private static let pairingCodeExpiry: TimeInterval = 120 // 2 minutes

    private let queue = DispatchQueue(label: "app.agentisland.watch.http", qos: .userInitiated)
