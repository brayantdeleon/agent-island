...
public struct HookHealthReport: Equatable, Sendable {
    ...
    public enum Issue: Equatable, Sendable, CustomStringConvertible {
        ...
        /// Other hooks detected alongside Agent-Island hooks (informational).
...
public enum HookHealthCheck {
    ...
    public static func checkClaude(
        ...
        // 3. Check manifest
        if fileManager.fileExists(atPath: settingsPath),
           hasAgentIslandHooks(in: settingsURL, fileManager: fileManager) {
    ...
    public static func checkCodex(
        ...
        // 3. Check manifest
        if fileManager.fileExists(atPath: hooksPath),
           hasAgentIslandHooks(in: hooksURL, fileManager: fileManager) {
    ...
    public static func checkOpenCode(
        ...
        let pluginFileURL = pluginsDir.appendingPathComponent("agent-island.js")
    ...
    /// Checks whether any Agent-Island managed hooks exist in a config file.
    private static func hasAgentIslandHooks(in url: URL, fileManager: FileManager) -> Bool {
