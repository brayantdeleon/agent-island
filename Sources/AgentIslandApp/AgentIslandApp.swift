...
final class AgentIslandAppDelegate: NSObject, NSApplicationDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ProcessInfo.processInfo.disableAutomaticTermination(
            "Agent-Island should remain active while monitoring local agent sessions."
        )
        ...
        DispatchQueue.main.async { [self] in
            ...
            AgentIslandAppDelegate.hideAllAppWindows()
...
struct AgentIslandApp: App {
    @NSApplicationDelegateAdaptor(AgentIslandAppDelegate.self)
    private var appDelegate

    @Environment(\.openWindow) private var openWindow

    var body: some Scene {
        Window("Agent-Island Settings", id: "settings") {
