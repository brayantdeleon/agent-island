// Agent-Island plugin for OpenCode
// Bridges OpenCode events to the Agent-Island desktop app via Unix socket.
// Install: copy to ~/.config/opencode/plugins/agent-island.js
import { connect } from "net";
import { appendFileSync } from "fs";
import { homedir } from "os";

const DEBUG_LOG = "/tmp/agent-island-opencode-debug.log";
function debugLog(msg) {
  try { appendFileSync(DEBUG_LOG, `[${new Date().toISOString()}] ${msg}\n`); } catch {}
}

const SOCKET_PATH =
  process.env.AGENT_ISLAND_SOCKET_PATH ||
  `${process.env.HOME || homedir()}/Library/Application Support/AgentIsland/bridge.sock`;
  ...
  return {
    "event": async ({ event }) => {
      try {
        ...
        // Fail open: if Agent-Island is unavailable, don't block OpenCode
      }
    },

    "shell.env": async (input, output) => {
      output.env.AGENT_ISLAND_ACTIVE = "1";
