...
[`notchi`](https://github.com/sk-ruban/notchi) is a strong adjacent reference for Agent-Island:
...
Agent-Island should learn from that execution without collapsing into a Claude-specific mascot app.
...
That is a useful product pattern for Agent-Island:

1. ambient awareness in the notch
2. actionable detail on expand
3. local transport and setup automation underneath

## Where Agent-Island Should Differ

Notchi is optimized for Claude Code presence and delight. Agent-Island should stay optimized for control.

Agent-Island should keep these distinctions:
...
Agent-Island already has the beginning of the Codex side:

- [Sources/AgentIslandCore/CodexHooks.swift](/Users/wangruobing/Personal/agent-island/Sources/AgentIslandCore/CodexHooks.swift)
- [Sources/AgentIslandHooks/main.swift](/Users/wangruobing/Personal/agent-island/Sources/AgentIslandHooks/main.swift)

The next step is to add a native installer that:

- detects Codex config presence
- installs the `AgentIslandHooks` executable or a thin wrapper script
...
Agent-Island should do a control-oriented version of that:
...
Agent-Island can adopt that idea in a more neutral form:
...
Notchi demonstrates that a narrow first integration is enough to ship. Agent-Island should still keep its bridge generic so Claude Code or other agents can be added later as adapters.

The current shared model and bridge transport already point in the right direction:

- [Sources/AgentIslandCore/AgentEvent.swift](/Users/wangruobing/Personal/agent-island/Sources/AgentIslandCore/AgentEvent.swift)
- [Sources/AgentIslandCore/BridgeTransport.swift](/Users/wangruobing/Personal/agent-island/Sources/AgentIslandCore/BridgeTransport.swift)
- [Sources/AgentIslandCore/SessionState.swift](/Users/wangruobing/Personal/agent-island/Sources/AgentIslandCore/SessionState.swift)
...
That would capture the highest-leverage lessons from Notchi while staying aligned with Agent-Island's stated product direction.
...
Those features may help Notchi's product identity, but they are not on Agent-Island's current critical path.
