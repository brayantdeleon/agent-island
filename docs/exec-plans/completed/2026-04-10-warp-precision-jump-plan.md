...
**Goal:** When a user clicks jump on a Warp-hosted Claude or Codex session in Agent-Island, automatically focus the exact Warp tab running that session (one click, no manual tab scanning).
...
| Path | Responsibility |
|---|---|
| `Sources/AgentIslandCore/WarpSQLiteReader.swift` | Read-only SQLite queries against Warp's database. Exposes `lookupPaneUUID(forCwd:)`, `currentFocusedPaneUUID()`, `tabCountInActiveWindow()`. All failures return nil/0 and log once. |
| `Sources/AgentIslandApp/KeystrokeInjector.swift` | Thin wrapper over `CGEventCreateKeyboardEvent`/`CGEventPost` exposing `sendCmdShiftRightBracket()`. Protocol + default impl for injection in tests. |
| `Sources/AgentIslandApp/AccessibilityPermissionChecker.swift` | One-function wrapper around `AXIsProcessTrustedWithOptions` so we can detect + graceful-degrade when Accessibility permission is missing. |
| `Tests/AgentIslandCoreTests/WarpSQLiteReaderTests.swift` | Unit tests using a synthetic warp.sqlite temp-file fixture that reproduces the exact minimal schema we query. |

**Modified files**

| Path | Change |
|---|---|
| `Sources/AgentIslandCore/AgentSession.swift` | Add `warpPaneUUID: String?` field to `JumpTarget` struct (and its init). Backward-compat Codable via `decodeIfPresent`. |
| `Sources/AgentIslandCore/ClaudeHooks.swift` | Add `warpPaneUUID` field to `ClaudeHookPayload`. Populate it in `withRuntimeContext` when `terminalApp == "Warp"`. Pass it through `defaultJumpTarget`. |
| `Sources/AgentIslandCore/CodexHooks.swift` | Same three changes as ClaudeHooks. |
| `Sources/AgentIslandApp/TerminalJumpService.swift` | Add `WarpFocusedPaneReader` / `WarpTabCountReader` / `WarpKeystroker` typealiases + injection points. Add `jumpToWarpPane(_:)` private method. Register `case "dev.warp.Warp-Stable"` in the `jump(to:)` dispatcher. |
| `Tests/AgentIslandCoreTests/ClaudeHooksTests.swift` | Add tests verifying `withRuntimeContext` populates `warpPaneUUID` when reader stub returns a value, skips lookup when `terminalApp != "Warp"`, and leaves payload unchanged on nil return. |
| `Tests/AgentIslandCoreTests/CodexHooksTests.swift` | Same. |
| `Tests/AgentIslandAppTests/TerminalJumpServiceTests.swift` | Add tests for `jumpToWarpPane` covering: already-on-target (zero keystrokes, Warp activated), reaches-target-after-N-cycles, never-reaches-target (caps out gracefully), nil target (degrades to app activation). |
...
**Files:**
- Modify: `Sources/AgentIslandCore/AgentSession.swift:114-143`
- Test: `Tests/AgentIslandCoreTests/SessionStateTests.swift` (add one Codable round-trip test)

- [ ] **Step 1: Write failing round-trip Codable test**

Add to `Tests/AgentIslandCoreTests/SessionStateTests.swift` (inside the existing test struct):
...
Edit `Sources/AgentIslandCore/AgentSession.swift` lines 114-143. The existing struct already has `tmuxTarget` and `tmuxSocketPath` — add `warpPaneUUID` alongside them:
...
```bash
git add Sources/AgentIslandCore/AgentSession.swift Tests/AgentIslandCoreTests/SessionStateTests.swift
...
**Files:**
- Modify: `Sources/AgentIslandCore/ClaudeHooks.swift:355-403` (add field + CodingKey) and the `defaultJumpTarget` computed property near line 677
- Modify: `Sources/AgentIslandCore/CodexHooks.swift` (equivalent field, CodingKey, defaultJumpTarget)
- Test: `Tests/AgentIslandCoreTests/ClaudeHooksTests.swift`, `Tests/AgentIslandCoreTests/CodexHooksTests.swift`

- [ ] **Step 1: Write failing test for Claude**

Add to `Tests/AgentIslandCoreTests/ClaudeHooksTests.swift`:
...
Add to `Tests/AgentIslandCoreTests/CodexHooksTests.swift`:
...
In `Sources/AgentIslandCore/ClaudeHooks.swift`:
...
In `Sources/AgentIslandCore/CodexHooks.swift`, repeat all six sub-steps from Step 4 against the Codex payload type. The `terminalApp` field is already there (added in an earlier change); `warpPaneUUID` slots in beside it.
...
```bash
git add Sources/AgentIslandCore/ClaudeHooks.swift Sources/AgentIslandCore/CodexHooks.swift Tests/AgentIslandCoreTests/ClaudeHooksTests.swift Tests/AgentIslandCoreTests/CodexHooksTests.swift
...
**Files:**
- Create: `Sources/AgentIslandCore/WarpSQLiteReader.swift`
- Test: `Tests/AgentIslandCoreTests/WarpSQLiteReaderTests.swift`

- [ ] **Step 1: Create the test file with a failing init test**

Create `Tests/AgentIslandCoreTests/WarpSQLiteReaderTests.swift`:
...
```bash
git add Sources/AgentIslandCore/WarpSQLiteReader.swift Tests/AgentIslandCoreTests/WarpSQLiteReaderTests.swift
...
**Files:**
- Modify: `Sources/AgentIslandCore/WarpSQLiteReader.swift`
- Modify: `Tests/AgentIslandCoreTests/WarpSQLiteReaderTests.swift`

- [ ] **Step 1: Add a SQLite fixture helper to the test file**

Append to `Tests/AgentIslandCoreTests/WarpSQLiteReaderTests.swift` **outside** the test struct:
...
enum WarpSQLiteFixture {
    ...
    struct Scenario {
        ...
        /// tab 2 cwd=agent-island (pane uuid BBBB...), tab 3 cwd=/tmp (pane uuid CCCC...).
        /// Claude was launched in giftcard from Warp shell session 1001 and in agent-island from 1002.
        static let threeTabsTwoClaudes = Scenario(rows: [
            ...
            "INSERT INTO terminal_panes (id, uuid, cwd) VALUES " +
                ...
                "(2, x'BBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBB', '/Users/u/agent-island')," +
            ...
            "INSERT INTO commands (id, session_id, command, pwd) VALUES " +
                ...
                "(200, 1002, 'claude --dangerously-skip-permissions', '/Users/u/agent-island');",
...
func lookupPaneUUIDReturnsUppercaseHexForClaudeInKnownCwd() throws {
    ...
    let uuid = reader.lookupPaneUUID(forCwd: "/Users/u/agent-island")
...
In `Sources/AgentIslandCore/WarpSQLiteReader.swift`, append a method to the struct:
...
func currentFocusedPaneUUIDReturnsActiveTabsUUID() throws {
    ...
    // Scenario has active_tab_index = 1 which is tab id 2 (agent-island).
    let reader = WarpSQLiteReader(databasePath: tmp)
...
**Files:**
- Modify: `Sources/AgentIslandCore/ClaudeHooks.swift` (in `withRuntimeContext`, around line 920-940 where `inferTerminalApp` is called)
- Modify: `Tests/AgentIslandCoreTests/ClaudeHooksTests.swift`
...
In `Sources/AgentIslandCore/ClaudeHooks.swift`, find the convenience `withRuntimeContext()` (no args) that calls the full one, and find the full one that takes `environment`, `currentTTYProvider`, `terminalLocatorProvider`. Modify the full version:
...
```bash
git add Sources/AgentIslandCore/ClaudeHooks.swift Tests/AgentIslandCoreTests/ClaudeHooksTests.swift
...
**Files:**
- Modify: `Sources/AgentIslandCore/CodexHooks.swift` (find its `withRuntimeContext` equivalent)
- Modify: `Tests/AgentIslandCoreTests/CodexHooksTests.swift` (there may not be one yet — if so, see Step 1)

- [ ] **Step 1: Locate Codex's runtime-context method**

Read `Sources/AgentIslandCore/CodexHooks.swift` and find the method equivalent to Claude's `withRuntimeContext`. Based on earlier exploration it's around line 389-460 and invokes `inferTerminalApp(from: environment)` (line 394). Confirm the method signature before continuing.

- [ ] **Step 2: Write failing test**

If `Tests/AgentIslandCoreTests/CodexHooksTests.swift` does not exist yet, create it with the standard header:
...
In `Sources/AgentIslandCore/CodexHooks.swift`, modify the full `withRuntimeContext` (around line 386) and its convenience overload (around line 378):
...
```bash
git add Sources/AgentIslandCore/CodexHooks.swift Tests/AgentIslandCoreTests/CodexHooksTests.swift
...
**Files:**
- Create: `Sources/AgentIslandApp/KeystrokeInjector.swift`
- Test: `Tests/AgentIslandAppTests/KeystrokeInjectorTests.swift`

- [ ] **Step 1: Write the failing test**

Create `Tests/AgentIslandAppTests/KeystrokeInjectorTests.swift`:
...
```bash
git add Sources/AgentIslandApp/KeystrokeInjector.swift Tests/AgentIslandAppTests/KeystrokeInjectorTests.swift
...
**Files:**
- Create: `Sources/AgentIslandApp/AccessibilityPermissionChecker.swift`
- Test: `Tests/AgentIslandAppTests/AccessibilityPermissionCheckerTests.swift`

- [ ] **Step 1: Write failing test**

Create `Tests/AgentIslandAppTests/AccessibilityPermissionCheckerTests.swift`:
...
```bash
git add Sources/AgentIslandApp/AccessibilityPermissionChecker.swift Tests/AgentIslandAppTests/AccessibilityPermissionCheckerTests.swift
...
**Files:**
- Modify: `Sources/AgentIslandApp/TerminalJumpService.swift` — add new typealiases, init parameters, the `jumpToWarpPane` private method, AND the dispatcher case.
- Modify: `Tests/AgentIslandAppTests/TerminalJumpServiceTests.swift`
...
// defined at file scope in Tests/AgentIslandAppTests/KeystrokeInjectorTests.swift
...
Before running Step 2, confirm that `KeystrokeInjectorSpy` in `Tests/AgentIslandAppTests/KeystrokeInjectorTests.swift` is at file scope (not nested inside `final class KeystrokeInjectorTests`) and has `internal` access. If Task 8 placed it nested or fileprivate, promote it before continuing this task.
...
In `Sources/AgentIslandApp/TerminalJumpService.swift`, just after the existing typealiases (around line 10), add:
...
```bash
git add Sources/AgentIslandApp/TerminalJumpService.swift Tests/AgentIslandAppTests/TerminalJumpServiceTests.swift
...
If `/Applications/Agent-Island.app` is running, quit it first (`osascript -e 'quit app "Agent-Island"'`) so there's no bridge-socket collision.

- [ ] **Step 2: Grant Accessibility permission**

Open **System Settings → Privacy & Security → Accessibility**. If `AgentIslandApp` is listed, toggle it on. If not, trigger a jump attempt from Agent-Island first (which will cause macOS to add it to the list) and come back.

- [ ] **Step 3: Set up the multi-tab scenario**

1. Open Warp.
2. Open 4 new tabs, each in a **different directory**:
   - `cd ~/Developer/agent-island && claude`
...
1. Open Agent-Island's session list (notch / top-bar).
...
2. In Agent-Island, click jump for that same session.
...
1. **No permission**: temporarily disable AgentIslandApp in System Settings → Accessibility. Click jump. Expected: Warp activates, no cycling, Agent-Island toast/log says "Grant Accessibility permission to enable precision jump."
2. **Re-enable permission** and continue.
3. **Close a Claude tab** while the session is still in Agent-Island's list. Click jump for the closed session. Expected: cycling runs the full cap, then returns "Activated Warp but could not confirm precision focus." — never crashes.
...
After committing, run a final smoke test: `swift run AgentIslandApp` with three Claude tabs in Warp, click jump on each in turn, verify all three land on the right tab. If anything regressed, investigate before pushing.
