...
<p align="center">
  <img src="docs/images/demo.gif" alt="Agent-Island 运行效果" width="720">
</p>
...
Agent-Island 驻留在 Mac 的**刘海区域**（或顶部栏），为你的 AI coding agents 提供实时控制面板——会话状态、权限审批、一键跳回正确的终端。全程不打断你的工作流。
...
首次启动时，Agent-Island 会自动发现活跃的 agent 会话并启动 live bridge。Hook 安装在 app 内的**设置**窗口管理。
...
| Target | 角色 |
|---|---|
| **AgentIslandApp** | SwiftUI + AppKit shell——菜单栏、覆盖面板、设置 |
| **AgentIslandCore** | 共享库——模型、bridge 传输（Unix socket IPC）、hooks、会话持久化 |
| **AgentIslandHooks** | 轻量 CLI，由 agent hooks 调用，通过 Unix socket 转发 payload |
| **AgentIslandSetup** | 安装器 CLI，管理 `~/.codex/config.toml` 和 hook entries |
...
1. 收集我的环境信息：
   ...
   - 检查 Agent-Island 是否在运行：`ps aux | grep -i "agent.island\|AgentIslandApp" | grep -v grep`
...
`Agent-Island` 会在刘海区域或顶部栏放置一个轻量控制界面，让你可以在不中断当前 flow 的前提下观察 live coding agents、跟踪会话进度，并快速跳回正确的 terminal 上下文。

### 为什么会有这个产品

AI coding 正在成为日常开发流程的一部分，但围绕它的控制层仍然经常意味着把你的机器交给一个闭源、收费的 app。

`Agent-Island` 选择了相反的路线：
...
- **Kimi CLI** — 基于 hook 的集成，通过 `~/.kimi/config.toml` 的 `[[hooks]]` 数组配置（Moonshot AI）。Kimi 的 hook payload 与 Claude Code 字段兼容，Agent-Island 复用 Claude 解码路径，仅新增了 TOML installer。订阅 `SessionStart`、`UserPromptSubmit`、`Stop`、`Notification`、`PreToolUse`、`PostToolUse`。需要 Kimi CLI Hooks Beta。使用 `--source kimi` 调用 hooks binary。可以从设置窗口管理安装，或通过 CLI：

  ```sh
  swift run AgentIslandSetup installKimi    # 把受管 [[hooks]] 条目写入 ~/.kimi/config.toml
  swift run AgentIslandSetup statusKimi     # 查看受管 hooks 是否已安装
  swift run AgentIslandSetup uninstallKimi  # 移除受管条目，保留用户自定义的 [[hooks]]
  ```
...
该脚本会创建 `output/package/Agent-Island.app` 和 `output/package/Agent-Island.zip`。传入 `AGENT_ISLAND_SIGN_IDENTITY` 可以签名。详见 [docs/packaging.md](docs/packaging.md)。
...
设置窗口展示来自 `~/.codex` 的实时 Codex hook 安装状态，并可直接安装或卸载受管 hook entries。安装过程会把 helper 复制到 `~/Library/Application Support/AgentIsland/bin/AgentIslandHooks`，repo 重命名不会破坏已有 hooks。
...
Claude usage 设置可在 app 设置窗口启用，保持 opt-in。bridge 会把受管 `statusLine.command` 写入 `~/.agent-island/bin/agent-island-statusline`，把 `rate_limits` 缓存到 `/tmp/agent-island-rl.json`，不会自动覆盖已有的自定义 status line。
