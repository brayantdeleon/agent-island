...
struct CodexSessionTrackingTests {
    ...
    func codexSessionStoreRoundTripsTrackedSessions() throws {
        let rootURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("agent-island-tracking-\(UUID().uuidString)", isDirectory: true)
        ...
        let records = [
            CodexTrackedSessionRecord(
                ...
                jumpTarget: JumpTarget(
                    ...
                    paneTitle: "codex ~/Personal/agent-island"
                ),
    ...
    func codexSessionStoreLoadsLegacyRecordsWithoutAttachmentState() throws {
        let rootURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("agent-island-legacy-tracking-\(UUID().uuidString)", isDirectory: true)
        let fileURL = rootURL.appendingPathComponent("session-terminals.json")
        let store = CodexSessionStore(fileURL: fileURL)
        ...
        CodexRolloutReducer.apply(
            line: rolloutLine(
                ...
                payload: [
                    ...
                    "changes": [
                        "Sources/AgentIslandCore/CodexSessionTracking.swift": [:],
                    ],
                ]
            ),
            to: &snapshot
    ...
    func codexRolloutWatcherTracksAppendedLines() async throws {
        let rootURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("agent-island-rollout-\(UUID().uuidString)", isDirectory: true)
    ...
    func codexRolloutWatcherBootstrapsPromptMetadataFromHeadWhenTailMissesIt() async throws {
        let rootURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("agent-island-rollout-head-bootstrap-\(UUID().uuidString)", isDirectory: true)
        let rolloutURL = rootURL.appendingPathComponent("rollout.jsonl")
        try FileManager.default.createDirectory(at: rootURL, withIntermediateDirectories: true)
    ...
    func codexRolloutWatcherBootstrapsFromBoundedTailWindow() async throws {
        let rootURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("agent-island-rollout-tail-\(UUID().uuidString)", isDirectory: true)
        let rolloutURL = rootURL.appendingPathComponent("rollout.jsonl")
        try FileManager.default.createDirectory(at: rootURL, withIntermediateDirectories: true)
    ...
    func codexRolloutDiscoveryFindsRecentSessionsFromLocalRollouts() throws {
        let rootURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("agent-island-discovery-\(UUID().uuidString)", isDirectory: true)
        ...
        #expect(records.first?.jumpTarget?.workspaceName == "agent-island")
        #expect(records.first?.jumpTarget?.paneTitle == "Rollout discovery cleanup")
        #expect(records.first?.jumpTarget?.workingDirectory == "/Users/wangruobing/Personal/agent-island")
    ...
    func codexRolloutDiscoveryStreamsRolloutsLargerThanReadChunk() throws {
        ...
        let rootURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("agent-island-discovery-stream-\(UUID().uuidString)", isDirectory: true)
    ...
    func codexRolloutDiscoveryHandlesTrailingLineWithoutNewline() throws {
        ...
        // than dropping it.
        let rootURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("agent-island-discovery-trailing-\(UUID().uuidString)", isDirectory: true)
        let rolloutDirectoryURL = rootURL.appendingPathComponent("2026/04/02", isDirectory: true)
