...
struct SessionStateTests {
    ...
    func actionableStateResolvedClearsWaitingForApproval() {
        ...
        #expect(state.session(id: "claude-approval")?.summary == "Approval was handled outside Agent-Island.")
    }
    ...
    func codexHookInstallerMergesManagedGroupsWithoutDroppingUnrelatedHooks() throws {
        ...
        let mutation = try CodexHookInstaller.installHooksJSON(
            ...
            hookCommand: "'/tmp/AgentIslandHooks'"
        )
        ...
        #expect(stopCommands.contains("'/tmp/AgentIslandHooks'"))
    ...
    @Test
    func codexHookInstallerInstallReplacesLegacyAgentIslandCommands() throws {
        let existing = """
        {
          "hooks": {
            "PreToolUse": [
              {
                ...
                "hooks": [
                  {
                    "type": "command",
                    "command": "'/Users/test/.agent-island/bin/agent-island-bridge' --source codex"
                  },
                  {
                    "type": "command",
            ...
            "Stop": [
              {
                "hooks": [
                  ...
                  {
                    "type": "command",
                    "command": "'/tmp/old-debug/AgentIslandHooks'"
                  },
                  {
                    "type": "command",
                    "command": "/usr/bin/true"
        ...
        let mutation = try CodexHookInstaller.installHooksJSON(
            existingData: existing,
            hookCommand: "'/tmp/new-release/AgentIslandHooks'"
        )
        ...
        #expect(!stopCommands.contains("'/Users/test/.agent-island/bin/agent-island-bridge' --source codex"))
        #expect(!stopCommands.contains("'/tmp/old-debug/AgentIslandHooks'"))
    }
    ...
    func codexHookInstallerUninstallRemovesOnlyManagedHooks() throws {
            ...
            "Stop": [
              {
                "hooks": [
                  {
                    "type": "command",
                    "command": "'/tmp/AgentIslandHooks'",
                    "statusMessage": "Managed by Agent-Island"
                  },
                  {
                    "type": "command",
                    "command": "/usr/bin/true",
        ...
        let mutation = try CodexHookInstaller.uninstallHooksJSON(
            existingData: existing,
            managedCommand: "'/tmp/AgentIslandHooks'"
        )
    ...
    func codexHookPayloadCarriesSessionLocatorIntoJumpTarget() {
        let payload = CodexHookPayload(
            ...
            terminalTitle: "codex ~/P/agent-island",
            transcriptPath: nil
        )
        ...
        #expect(jumpTarget.paneTitle == "codex ~/P/agent-island")
    }
